...
def fast_hist(a, b, n):
    #   One dimension array (H×W)
    k = (a >= 0) & (a < n)
    #   np.bincount calculate the number of times，return(n, n)
...
def compute_mIoU(gt_dir, pred_dir, png_name_list, num_classes, name_classes=None):  
    ...
    for ind in range(len(gt_imgs)): 
        ...
        pred = np.array(np.load(pred_imgs[ind]))
        pred_h, pred_w = pred.shape

        label = np.array(np.load(gt_imgs[ind]))
        h, w = label.shape

        pred = scipy.ndimage.zoom(pred, (256 / pred_h, 256 / pred_w), output=None,
