...
class EvalCallback():
    ...
    def get_miou_png(self, image):
        ...
        # image_data  = np.expand_dims(np.expand_dims(np.array(image, np.float32), 0), 0)
    ...
    def on_epoch_end(self, epoch, model_eval):
        if epoch % self.period == 0 and self.eval_flag:
            ...
            for image_id in tqdm(self.image_ids):
                ...
                image_path  = os.path.join(self.dataset_path, "Images/"+image_id+".npy")
                image       = np.load(image_path)
                h, w, _ = image.shape
                image = scipy.ndimage.zoom(image, (int(self.input_shape[0]) / h, int(self.input_shape[1]) / w, 1),
                                         output=None,
